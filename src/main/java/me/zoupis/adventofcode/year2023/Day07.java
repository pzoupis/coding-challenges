...
public class Day07 {
  ...
  private static long part2() {
    ...
    for (String line : input) {
      ...
      Type type = identifyTypeWithJoker(hand);
    ...
    return calculateTotalWinningsWithJoker(map);
  }
  ...
  public static long calculateTotalWinningsWithJoker(Map<Type, List<Map<String, Integer>>> maps) {
    int sum = 0;
    int rank = 1;

    for (Type type : Type.values()) {
      if (maps.containsKey(type)) {
        maps.get(type).sort(new CardComparatorWithJoker());
        for (Map<String, Integer> card : maps.get(type)) {
          sum += card.values().stream().mapToInt(Integer::valueOf).sum() * rank;
          rank++;
        }
  ...
  public static Type identifyTypeWithJoker(String hand) {
    var map = frequencyMap(hand.chars().mapToObj(c -> (char) c));
    ...
    return switch (map.size()) {
      ...
      case 2 -> {
        ...
        else if (map.containsKey('J') && map.get('J') >= 2) {
          yield Type.FIVE_OF_A_KIND;
        } else {
      ...
      default -> {
        if (map.containsKey('J')) {
          ...
          yield Type.HIGH_CARD;
        }
      }
    };
  }

  public enum CardStrengthWithJoker {
    J,
    T,
    Q,
    K,
  ...
  static class CardComparatorWithJoker implements Comparator<Map<String, Integer>> {

    @Override
    public int compare(Map<String, Integer> o1, Map<String, Integer> o2) {
      String key1 = o1.keySet().stream().toList().getFirst();
      String key2 = o2.keySet().stream().toList().getFirst();
      ...
      for (int i = 0; i < 5; i++) {
        ...
        if (Character.isDigit(key1.charAt(i)) && Character.isDigit(key2.charAt(i))) {
          return key1.compareTo(key2);
        } else if (Character.isDigit(key1.charAt(i)) && key2.charAt(i) == 'J') {
          return 1;
        } else if (key1.charAt(i) == 'J' && Character.isDigit(key2.charAt(i))) {
          return -1;
        }
        else if (Character.isDigit(key1.charAt(i)) && !Character.isDigit(key2.charAt(i))) {
          return -1;
        ...
        CardStrengthWithJoker c1 = CardStrengthWithJoker.valueOf(String.valueOf(key1.charAt(i)));
        CardStrengthWithJoker c2 = CardStrengthWithJoker.valueOf(String.valueOf(key2.charAt(i)));
        return c1.compareTo(c2);
      }
      throw new IllegalArgumentException();
    }
